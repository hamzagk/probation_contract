#!/usr/bin/env python
"""
Send manual probation expiration list email for FEE (Faculty of Electrical Engineering)
To: ikramullah@giki.edu.pk
...
def send_fee_probation_email():
    """Send probation report for FEE department"""
    print("=== Sending FEE Probation Report ===")
    
    # FEE department names in the database
    fee_departments = ['FEE', 'Faculty of Electrical Engineering', 'Electrical Engineering']
    
    # Find employees with probation ending within 30 days (includes recently ended)
    # Look back 15 days and forward 30 days from today
    past_date = date.today() - timedelta(days=15)
    future_date = date.today() + timedelta(days=30)
    
    employees_queryset = Employee.objects.filter(
        department__name__in=fee_departments,
        end_date__range=[past_date, future_date],
        probation_status__in=['Active', 'Ending Soon', 'Completed']
    ).order_by('end_date')
    
    if not employees_queryset.exists():
        print("No FEE employees found with probation ending within 30 days from today.")
        # Let's also check without date filter to see all FEE employees
        all_fee = Employee.objects.filter(department__name__in=fee_departments)
        print(f"Total FEE employees in database: {all_fee.count()}")
        if all_fee.exists():
            print("\nAll FEE employees:")
            for emp in all_fee:
    ...
    print(f"Found {employees_queryset.count()} FEE employee(s) with probation ending soon")
    print()
    ...
    context = {
        ...
        'report_title': 'Probation Report: FEE (Faculty of Electrical Engineering)',
        'hr_contact': getattr(settings, 'HR_EMAIL', 'HR Department'),
        'report_period': 'Next 30 days',
        'department_name': 'FEE (Faculty of Electrical Engineering)',
    }
    ...
    # Email recipients
    to_email = ['ikramullah@giki.edu.pk']  # FEE contact
    cc_email = ['muhammad.hamza@giki.edu.pk']  # CC to Muhammad Hamza
    
    subject = f"Probation Report: FEE (Faculty of Electrical Engineering) - {len(employees)} Employee(s) ({date.today().strftime('%B %d, %Y')})"
    ...
    email = EmailMultiAlternatives(
        subject=subject,
        body=f"This is an HTML email report containing a list of FEE employees whose probation periods are ending soon (within the next 30 days). Please view it in a compatible email client.",
    ...
    try:
        email.send()
        print("[OK] SUCCESS: Email sent to FEE with CC to Muhammad Hamza")
